...
router.post("/:id/soccer-players", async (req, res, next) => {
  try {
    ...
    if (!player) {
      await SoccerPlayer.create(req.body);
    } else if (player) {
...
// router.put("/:id/soccer-players", async (req, res, next) => {
//   try {
//     const player = await SoccerPlayer.findOne({
//       where: {
//         userId: req.params.id,
...
router.delete("/:id/soccer-players", async (req, res, next) => {
  try {
    const allPlayers = await SoccerPlayer.findAll({
      where: {
        userId: req.params.id,
